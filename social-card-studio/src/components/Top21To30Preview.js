...
export function renderTop21To30Preview() {
  return `
    <div class="tr-preview" id="tr-preview-21">
      ...
      <div class="safe-zone-wrapper" style="transform: scale(0.99); transform-origin: center center; flex: 1; display: flex; flex-direction: column; width: 100%; height: 100%; z-index: 2;">
        <div class="tr-header">
          <div class="tr-title-group">
            <div class="tr-subtitle" id="prev-tr-subtitle-21"></div>
            <div class="tr-title" id="prev-tr-title-21"></div>
          </div>
          <div class="tr-meta">
            <div class="tr-period" id="prev-tr-period-21"></div>
            <div class="tr-date" id="prev-tr-date-21"></div>
          </div>
        </div>

        <div class="tr-content">
          <div class="tr-list" id="prev-tr-list-21">
...
export function initTop21To30Preview() {
  const render = (state) => {
    const tr = state.topRanking;
    
    const titleEl = document.getElementById('prev-tr-title-21');
    const subtitleEl = document.getElementById('prev-tr-subtitle-21');
    const periodEl = document.getElementById('prev-tr-period-21');
    const dateEl = document.getElementById('prev-tr-date-21');
    ...
    const container = document.getElementById('prev-tr-list-21');
    if (!container) return;

    const itemsToRender = tr.items.slice(20, 30);
    ...
    const previewWrapper = document.getElementById('tr-preview-21');
    ...
    if (container.children.length === itemsToRender.length) {
      itemsToRender.forEach((item, index) => {
        ...
        row.setAttribute('data-rank', index + 21);
      });
      createIcons({
