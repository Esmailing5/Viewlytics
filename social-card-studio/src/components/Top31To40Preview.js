...
export function renderTop31To40Preview() {
  return `
    <div class="tr-preview" id="tr-preview-31">
      ...
      <div class="safe-zone-wrapper" style="transform: scale(0.99); transform-origin: center center; flex: 1; display: flex; flex-direction: column; width: 100%; height: 100%; z-index: 2;">
        <div class="tr-header">
          <div class="tr-title-group">
            <div class="tr-subtitle" id="prev-tr-subtitle-31"></div>
            <div class="tr-title" id="prev-tr-title-31"></div>
          </div>
          <div class="tr-meta">
            <div class="tr-period" id="prev-tr-period-31"></div>
            <div class="tr-date" id="prev-tr-date-31"></div>
          </div>
        </div>

        <div class="tr-content">
          <div class="tr-list" id="prev-tr-list-31">
...
export function initTop31To40Preview() {
  const render = (state) => {
    const tr = state.topRanking;
    
    const titleEl = document.getElementById('prev-tr-title-31');
    const subtitleEl = document.getElementById('prev-tr-subtitle-31');
    const periodEl = document.getElementById('prev-tr-period-31');
    const dateEl = document.getElementById('prev-tr-date-31');
    ...
    const container = document.getElementById('prev-tr-list-31');
    if (!container) return;

    const itemsToRender = tr.items.slice(30, 40);
    ...
    const previewWrapper = document.getElementById('tr-preview-31');
    ...
    if (container.children.length === itemsToRender.length) {
      itemsToRender.forEach((item, index) => {
        ...
        row.setAttribute('data-rank', index + 31);
      });
      createIcons({
