        ...
        <table className="w-full border-collapse text-left">
          <thead>
            <tr className="border-b border-[var(--vl-border)]/45 text-[10px] font-bold text-[var(--vl-text-tertiary)] uppercase tracking-wider">
              <th className="p-4 pl-6 w-20">Posición</th>
              <th className="p-4">Canal</th>
              {tab === 'impact-total' && (
                <>
                  <th className="p-4 text-right">Impacto Total</th>
                  <th className="p-4 text-right">Vistas Videos</th>
                  <th className="p-4 text-right">Vistas Shorts</th>
                </>
              )}
              {tab === 'videos-largos' && (
                <>
                  <th className="p-4 text-right">Vistas Videos</th>
                  <th className="p-4 text-right">Videos Largos (30d)</th>
                </>
              )}
              {tab === 'shorts' && (
                <>
                  <th className="p-4 text-right">Vistas Shorts</th>
                  <th className="p-4 text-right">Shorts (30d)</th>
                </>
              )}
            </tr>
          </thead>
          <tbody className="divide-y divide-[var(--vl-border)]/20">
            {Array.from({ length: 5 }).map((_, idx) => (
              <tr key={idx} className="animate-pulse">
                ...
                {tab === 'impact-total' && (
                  <>
                    <td className="p-4 text-right"><div className="h-5 w-20 bg-white/[0.05] rounded ml-auto" /></td>
                    <td className="p-4 text-right"><div className="h-5 w-20 bg-white/[0.05] rounded ml-auto" /></td>
                    <td className="p-4 text-right"><div className="h-5 w-20 bg-white/[0.05] rounded ml-auto" /></td>
                  </>
                )}
                {tab === 'videos-largos' && (
                  <>
                    <td className="p-4 text-right"><div className="h-5 w-20 bg-white/[0.05] rounded ml-auto" /></td>
                    <td className="p-4 text-right"><div className="h-5 w-16 bg-white/[0.05] rounded ml-auto" /></td>
                  </>
                )}
                {tab === 'shorts' && (
                  <>
          ...
          {results.map((creator) => {
            ...
            return (
              ...
              >
                ...
                <td className="p-4">
                  <div className="flex items-center gap-3.5">
                    <div className="relative shrink-0">
                      ...
                      {isFirst && (
                        <span className="absolute bottom-0 right-0 w-2.5 h-2.5 bg-cyan-400 border-2 border-[var(--vl-bg-surface)] rounded-full shadow-[0_0_8px_#22d3ee]" />
                      )}
                    </div>
                    <div>
                      <h4 className="font-extrabold text-sm text-[var(--vl-text-primary)] group-hover:text-cyan-400 transition-colors flex items-center gap-1">
                        {creator.displayName}
                        <ShieldCheck className="w-4 h-4 text-blue-500 fill-blue-500/10 shrink-0" />
                      </h4>
                      <span className="text-[10px] font-semibold text-[var(--vl-text-tertiary)] uppercase tracking-wider">
                        YouTube
                      </span>
                    </div>
                  </div>
                ...
                {tab === 'impact-total' && (
                  <>
                    <td className="p-4 text-right font-black text-sm text-[var(--vl-text-primary)]">
                      {formatCompleteNumber(creator.impactTotal30d)}
                    </td>
                    <td className="p-4 text-right font-medium text-sm text-[var(--vl-text-secondary)]">
                ...
                {tab === 'videos-largos' && (
                  <>
                    <td className="p-4 text-right font-black text-sm text-[var(--vl-text-primary)]">
                      {formatCompleteNumber(creator.viewsVideos30d)}
                    </td>
                    <td className="p-4 text-right font-semibold text-sm text-[var(--vl-text-secondary)]">
                      {creator.videos30d}
                    </td>
                  </>
                )}
                ...
                {tab === 'shorts' && (
                  <>
                    <td className="p-4 text-right font-black text-sm text-[var(--vl-text-primary)]">
                      {formatCompleteNumber(creator.viewsShorts30d)}
                    </td>
                    <td className="p-4 text-right font-semibold text-sm text-[var(--vl-text-secondary)]">
                      {creator.shorts30d}
                    </td>
