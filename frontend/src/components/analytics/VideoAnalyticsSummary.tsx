...
import { TrendingUp, ThumbsUp, MessageCircle, Users, Activity } from 'lucide-react';
  ...
  const metrics = [
    {
      ...
      value: new Intl.NumberFormat('es-ES', { notation: "compact" }).format(avgViews),
    ...
    },
    {
      ...
      value: new Intl.NumberFormat('es-ES', { notation: "compact" }).format(totalLikes),
    ...
    },
    {
      ...
      value: new Intl.NumberFormat('es-ES', { notation: "compact" }).format(totalComments),
    ...
    <div className="vl-card-dashboard p-6 h-full flex flex-col border border-[var(--vl-border)] rounded-2xl bg-[var(--vl-bg-surface)]/60 backdrop-blur-md">
      <div className="flex items-center gap-2 mb-6">
        ...
        <h3 className="text-lg font-bold text-[var(--vl-text-primary)]">
          Impacto Reciente <span className="text-xs font-semibold text-[var(--vl-text-secondary)] lowercase">/ últimos 30 días</span>
        </h3>
      </div>
      ...
      <div className="space-y-3.5 flex-1 flex flex-col justify-center">
        {metrics.map((m, idx) => (
          ...
          >
            <div className={`w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 border ${m.bg} ${m.color}`}>
              {m.icon}
            </div>
            <div className="min-w-0 flex-1">
              <p className="text-[9px] font-bold text-[var(--vl-text-tertiary)] uppercase tracking-wider truncate">{m.label}</p>
              <div className="flex items-baseline gap-2 mt-0.5">
                <p className="text-base font-black text-[var(--vl-text-primary)] tracking-tight">{m.value}</p>
                {m.change && (
