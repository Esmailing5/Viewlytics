...
function StatCard({ metric, platform }: StatCardProps) {
  ...
  const badgeColorClass =
    ...
    'bg-[var(--vl-bg-primary)] text-[var(--vl-text-secondary)] border-[var(--vl-border)]';

  return (
    <div className="vl-metric-card-interactive border border-[var(--vl-border)]/55 bg-[var(--vl-bg-surface)]/40 backdrop-blur-md">
      <div className="flex-1 min-w-0">
        <p className="vl-metric-label">{metric.label}</p>
        <p className="vl-metric-value truncate">{metric.value}</p>
        {metric.id === 'platform' ? (
          ...
          metric.change && (
            <p className="vl-metric-comparison">
              ...
              <span className="text-[var(--vl-text-tertiary)] font-medium">
                vs 30d
              </span>
            </p>
          )
  ...
  const growthChartsData = useMemo(() => {
    if (!data)
      return {
        subsData: [],
        viewsData: [],
        subsChangePercent: 'Datos insuficientes',
        subsChangeAbsolute: '—',
        viewsChangePercent: 'Datos insuficientes',
        viewsChangeAbsolute: '—',
      };
    ...
    if (data.snapshots && data.snapshots.length >= 2) {
      ...
      subsChangeAbsolute =
        diff >= 0
          ? `+${new Intl.NumberFormat('es-ES', { notation: 'compact' }).format(diff)}`
          : `${new Intl.NumberFormat('es-ES', { notation: 'compact' }).format(diff)}`;
      ...
      viewsChangeAbsolute =
        diffViews >= 0
          ? `+${new Intl.NumberFormat('es-ES', { notation: 'compact' }).format(diffViews)}`
          : `${new Intl.NumberFormat('es-ES', { notation: 'compact' }).format(diffViews)}`;
  ...
  const dynamicMetrics = [
    {
      ...
      change:
        data.growth?.subscribers_change_pct !== undefined
    ...
    },
    {
      ...
      change:
        data.growth?.views_change_pct !== undefined
    ...
    },
    {
      ...
      change:
        data.growth?.videos_change_pct !== undefined
    ...
    <div className="space-y-6 px-4 sm:px-0">
          ...
          <div className="relative w-full p-4 sm:p-8 flex flex-col sm:flex-row items-center sm:items-end justify-between gap-6 z-10">
                ...
                <div className="flex items-center justify-center sm:justify-start gap-1.5 sm:gap-2 text-[10px] sm:text-xs text-gray-300 mt-2.5 font-semibold flex-wrap">
                  ...
                  <span className="whitespace-nowrap">
                    {new Intl.NumberFormat('es-ES', {
                      notation: 'compact',
                    }).format(data.profile?.subscribers || 0)}{' '}
                    Subs ▾
                  </span>
                  <span className="opacity-40">•</span>
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                className="vl-header-btn !text-white"
              >
              ...
              <button
                ...
                className={`vl-header-btn vl-header-btn-icon text-white ${isFavorited ? 'bg-yellow-500/20 border-yellow-500/40 text-yellow-500' : ''}`}
              >
                <Star
                  className={`w-4 h-4 ${isFavorited ? 'fill-yellow-500' : 'text-white/95'}`}
                />
              </button>
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div className="vl-card-dashboard p-4 md:p-5 border border-[var(--vl-border)] rounded-2xl bg-[var(--vl-bg-surface)]/60 backdrop-blur-md overflow-hidden">
          <div className="flex justify-between items-start mb-4 md:mb-6 gap-2">
            <div className="min-w-0">
              <h3 className="text-xs md:text-sm font-bold text-[var(--vl-text-secondary)]">
                Crecimiento de Suscriptores
              </h3>
              <p className="text-xl md:text-2xl font-black text-[var(--vl-text-primary)] mt-1">
                {growthChartsData.subsChangeAbsolute}
              </p>
            ...
            <span className="text-[10px] md:text-xs font-bold text-[var(--vl-success)] bg-green-500/10 px-2 py-0.5 rounded border border-green-500/20 whitespace-nowrap shrink-0">
              {growthChartsData.subsChangePercent}{' '}
              <span className="text-[var(--vl-text-secondary)] font-medium text-[9px] md:text-[10px]">
                vs 30d
              </span>
            </span>
          </div>
          ...
          <div className="h-[220px] w-full">
            {data.snapshots && data.snapshots.length >= 2 ? (
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart
                  data={growthChartsData.subsData}
              ...
              <div className="w-full h-full flex items-center justify-center border border-dashed border-[var(--vl-border)] rounded-xl bg-white/[0.01]">
                <p className="text-xs font-semibold text-[var(--vl-text-tertiary)] uppercase tracking-wider">
        ...
        <div className="vl-card-dashboard p-5 border border-[var(--vl-border)] rounded-2xl bg-[var(--vl-bg-surface)]/60 backdrop-blur-md">
          <div className="flex justify-between items-start mb-6">
            <div>
              <h3 className="text-sm font-bold text-[var(--vl-text-secondary)]">
                Impacto Total · últimos 30 días
              </h3>
              <p className="text-2xl font-black text-[var(--vl-text-primary)] mt-1">
                {new Intl.NumberFormat('en-US', {
                  notation: 'compact',
                  maximumFractionDigits: 2,
                }).format(data.growth?.total_views_30d || 0)}{' '}
                Views
              </p>
            </div>
          </div>
          ...
          <div className="h-[220px] w-full">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart
                data={recentViewsChartData}
                margin={{ top: 10, right: 10, left: -15, bottom: 5 }}
              >
                <defs>
                  <linearGradient id="cyanLong" x1="0" y1="0" x2="0" y2="1">
                    <stop offset="0%" stopColor="var(--vl-cyan)" />
                    <stop offset="100%" stopColor="var(--vl-cyan-hover)" />
                  </linearGradient>
                  <linearGradient id="purpleShort" x1="0" y1="0" x2="0" y2="1">
                    <stop offset="0%" stopColor="var(--vl-purple)" />
                    <stop offset="100%" stopColor="var(--vl-purple-hover)" />
                  </linearGradient>
                </defs>
                <CartesianGrid
                  strokeDasharray="4 4"
                ...
                <Tooltip
                  content={({ active, payload, label }) => {
                    if (active && payload && payload.length) {
                      const longVal =
                        payload.find((p: any) => p.dataKey === 'longViews')
                          ?.value || 0;
                      const shortVal =
                        payload.find((p: any) => p.dataKey === 'shortViews')
                          ?.value || 0;
                      return (
                        <div className="bg-[#0b0c10]/90 backdrop-blur-md border border-white/[0.08] rounded-xl px-3.5 py-2.5 shadow-2xl">
                          ...
                          <p className="font-extrabold text-white text-xs tracking-tight">
                            Videos largos:{' '}
                            <span className="text-[var(--vl-cyan)]">
                              {new Intl.NumberFormat('en-US', {
                                notation: 'compact',
                              }).format(longVal as number)}
                            </span>
                          </p>
                          <p className="font-extrabold text-white text-xs tracking-tight mt-0.5">
                            Shorts:{' '}
                            <span className="text-[var(--vl-purple)]">
                              {new Intl.NumberFormat('en-US', {
                                notation: 'compact',
                              }).format(shortVal as number)}
                            </span>
                          </p>
                        </div>
                      );
            ...
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-[var(--vl-text-primary)] font-bold">
                {new Intl.NumberFormat('en-US', {
                  notation: 'compact',
                  maximumFractionDigits: 2,
                }).format(data.growth?.views_30d || 0)}
              </span>
            </div>
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-[var(--vl-text-primary)] font-bold">
                {new Intl.NumberFormat('en-US', {
                  notation: 'compact',
                  maximumFractionDigits: 2,
                }).format(data.growth?.shorts_views_30d || 0)}
              </span>
            </div>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 items-start">
        ...
        <div className="lg:col-span-2 min-h-[420px] flex flex-col">
          <RecentVideosCard
            videos={
              data.recentVideos?.filter((v: any) => v.is_long || v.isLong) || []
            }
          />
        </div>
