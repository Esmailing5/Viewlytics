...
export function RecentVideosCard({ videos }: { videos: VideoProps[] }) {
  if (!videos || videos.length === 0) {
    return (
      <div className="dashboard-card p-6 flex items-center justify-center h-full">
        <p className="text-[var(--text-secondary)]">No hay videos recientes disponibles.</p>
      </div>
    );
  }
        ...
        {videos.map((video) => (
          <div key={video.id} className="flex gap-4 p-3 rounded-xl hover:bg-[var(--bg-main)] transition-colors border border-transparent hover:border-[var(--border-color)] group cursor-pointer">
            ...
            <div className="flex-1 min-w-0 flex flex-col justify-between">
              <h4 className="text-sm font-semibold text-[var(--text-primary)] line-clamp-2" title={video.title}>
                {video.title}
              </h4>
