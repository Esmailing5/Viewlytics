...
export function RecentVideosCard({ videos }: { videos: VideoProps[] }) {
  if (!videos || videos.length === 0) {
    return (
      <div className="vl-card-dashboard p-5 flex items-center justify-center h-full border border-[var(--vl-border)] rounded-2xl bg-[var(--vl-bg-surface)]/60">
        <p className="text-[var(--vl-text-secondary)] font-medium">
  ...
  const displayedVideos = videos.filter((v) => v.is_long === true).slice(0, 5);
    ...
    <div className="vl-card-dashboard p-5 flex flex-col h-full border border-[var(--vl-border)] rounded-2xl bg-[var(--vl-bg-surface)]/60 backdrop-blur-md justify-between">
      <div>
        <div className="flex items-center justify-between mb-5">
          <h3 className="text-base font-bold text-[var(--vl-text-primary)]">
            Últimos Videos
          </h3>
        </div>
            ...
            return (
              ...
              >
                ...
                <div className="flex-1 min-w-0 flex flex-col justify-between py-0.5">
                  ...
                  >
                    <h4
                      className="text-xs sm:text-sm font-bold text-[var(--vl-text-primary)] line-clamp-1 leading-snug group-hover:text-[var(--vl-red)] transition-colors duration-200"
                      title={video.title}
                    >
                      {video.title}
                    </h4>
                  ...
                  <div className="flex items-center gap-x-2.5 text-[10px] sm:text-xs text-[var(--vl-text-secondary)] font-semibold mt-1 flex-wrap">
                    <span className="flex items-center gap-0.5">
                      <Eye className="w-3 h-3 text-[var(--vl-text-tertiary)]" />{' '}
                    ...
                    <span className="flex items-center gap-0.5">
                      <ThumbsUp className="w-3 h-3 text-[var(--vl-text-tertiary)]" />{' '}
                      {new Intl.NumberFormat('es-ES', {
                        notation: 'compact',
                      }).format(video.likes)}
                    </span>
                    <span className="opacity-40">•</span>
                    <span className="flex items-center gap-0.5">
                      <MessageCircle className="w-3 h-3 text-[var(--vl-text-tertiary)]" />{' '}
                      {new Intl.NumberFormat('es-ES', {
                        notation: 'compact',
                      }).format(video.comments)}
                    </span>
                    <span className="opacity-40">•</span>
                    <span className="text-[var(--vl-text-tertiary)]">
                      {formatRealDate(video.published_at)}
                    </span>
                  </div>
                </div>
              </div>
